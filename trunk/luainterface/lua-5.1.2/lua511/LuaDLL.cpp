...
#undef lua_tostring
		...
		static String^ lua_tostring(IntPtr luaState, int index)
		{
			...
			else if(t == LuaTypes::LUA_TSTRING)
			{
				...
				UTF8Encoding^ encoding = gcnew UTF8Encoding(true, false);
				
				String^ clistr = gcnew String(str, 0, strlen, encoding);
		...
		{	
			...
			{
				/* TODO check for memory leak and optimize */
				UTF8Encoding^ encoding = gcnew UTF8Encoding( true,false );
				array<Byte>^ buff = gcnew array<Byte>(encoding->GetByteCount(str));
				encoding->GetBytes(str, 0, str->Length, buff, 0);

				int bufferSize = Marshal::SizeOf(buff[0]) * buff->Length;
				System::IntPtr buffer = Marshal::AllocHGlobal(bufferSize + 1);
				Marshal::Copy(buff, 0, buffer, bufferSize);
				char *cs = (char *) buffer.ToPointer();
				cs[bufferSize] = '\0';
...
#undef luaL_getmetatable
		...
		static int luaL_loadbuffer2(IntPtr luaState, array<Byte>^ buff, String^ name)
		{
			int bufferSize = Marshal::SizeOf(buff[0]) * buff->Length;
			System::IntPtr buffer = Marshal::AllocHGlobal(bufferSize + 1);
			Marshal::Copy(buff, 0, buffer, bufferSize);
			char *cs1 = (char *) buffer.ToPointer();
			cs1[bufferSize] = '\0';
			//printf("%s\n", cs1);
			char *cs2 = (char *) Marshal::StringToHGlobalAnsi(name).ToPointer();
